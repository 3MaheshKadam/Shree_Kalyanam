...
export default function UserLayout({ children }) {
  ...
  if (!user?.isVerified && !isProfilePage) {
    return (
      <div className="flex min-h-screen bg-gradient-to-br from-rose-50 via-white to-purple-50">
        <Sidebar mobileOpen={sidebarOpen} setMobileOpen={setSidebarOpen} />
        <div className="flex-1 p-6 overflow-y-auto flex items-center justify-center">
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.5 }}
            className="max-w-2xl w-full bg-white rounded-2xl shadow-xl overflow-hidden border border-pink-50"
          >
            <div className="p-8 text-center">
              <div className="mx-auto flex items-center justify-center h-20 w-20 rounded-full bg-pink-100 mb-6">
                <ShieldAlert className="h-10 w-10 text-pink-600" />
              </div>
              <h2 className="text-2xl font-bold text-slate-800 mb-2">Account Verification Required</h2>
              ...
              <div className="space-y-4 mb-8 text-left">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-500">Fill in all required details in your profile section</p>
                  </div>
                </div>
                <div className="flex items-start gap-3">
                  <XCircle className="h-5 w-5 text-rose-500 mt-0.5 flex-shrink-0" />
                  <div>
                    <h4 className="font-medium text-slate-800">Current Restrictions</h4>
                    <p className="text-sm text-slate-500">You cannot access other pages until verified</p>
                  </div>
                </div>
              </div>
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.98 }}
                onClick={() => router.push("/dashboard/profile/me")}
                className="inline-flex items-center justify-center px-6 py-3 bg-gradient-to-r from-pink-500 to-rose-600 text-white font-medium rounded-lg shadow-sm hover:shadow-md transition-all"
              >
                Complete My Profile
                <ArrowRight className="ml-2 h-4 w-4" />
              </motion.button>
  ...
  return (
    <div className="flex min-h-screen bg-gradient-to-br from-rose-50 via-white to-purple-50">
      ...
      <main className="flex-1 p-4 lg:p-6 overflow-y-auto relative">
        {/* Mobile header */}
        <div className="lg:hidden flex items-center justify-between mb-4 bg-white p-3 rounded-lg shadow-sm border border-pink-50">
          <button 
            onClick={() => setSidebarOpen(!sidebarOpen)}
            className="p-2 rounded-lg text-slate-600 hover:bg-pink-50"
          >
            <Menu className="w-5 h-5" />
          </button>
          <div className="flex items-center space-x-2">
            ...
            <span className="font-bold text-sm bg-gradient-to-r from-pink-600 to-rose-600 bg-clip-text text-transparent">
              shreekalyanam
            </span>
          </div>
        </div>
