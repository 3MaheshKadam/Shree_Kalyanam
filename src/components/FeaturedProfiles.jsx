...
export default function FeaturedProfiles() {
  ...
  useEffect(() => {
    const fetchProfiles = async () => {
      try {
        const res = await fetch('/api/users/fetchAllUsers?limit=10');
        const data = await res.json();

        if (data.success) {
          const formattedProfiles = data.data.map(user => ({
            id: user._id,
            name: user.name,
            age: calculateAge(user.dob),
            city: user.currentCity || 'Not Specified',
            profession: user.occupation || 'Professional',
            photo: user.profilePhoto || '/assets/placeholder-user.jpg',
            verified: user.isVerified,
            online: Math.random() > 0.7, // Simulated online status
            matchPercentage: Math.floor(Math.random() * (98 - 75) + 75), // Simulated match %
            education: user.education || 'Graduate',
            family: 'Nuclear Family' // Placeholder
          }));
    ...
    const interval = setInterval(() => {
      setCurrentStartIndex(prev => {
        const maxIndex = Math.max(0, profiles.length - 4);
            ...
            {getVisibleProfiles().map((profile, index) => {
              ...
              return (
                <div
                  ...
                  className={`group transition-all duration-700 transform ${isLoaded ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'
                    } hover:-translate-y-2`}
                  ...
                  `}>
                    ...
                    <div className="relative h-64 overflow-hidden">
                      ...
                      <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 flex gap-3 opacity-0 group-hover:opacity-100 transition-all duration-500 scale-90 group-hover:scale-100">
                        <button
                          ...
                          className={`p-3 rounded-full backdrop-blur-sm transition-all duration-300 ${likedProfiles.has(actualIndex)
                    ...
                    <div className="p-5">
                      ...
                      <div className="flex space-x-2">
                        ...
                        <button
                          onClick={(e) => toggleLike(actualIndex, e)}
                          className={`p-2.5 rounded-xl transition-all duration-300 ${likedProfiles.has(actualIndex)
          ...
          <div className="flex gap-2">
            {Array.from({ length: Math.ceil(profiles.length / cardsPerView) }, (_, i) => (
              <button
                ...
                className={`transition-all duration-300 rounded-full ${Math.floor(currentStartIndex / cardsPerView) === i
