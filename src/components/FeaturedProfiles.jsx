...
export default function FeaturedProfiles() {
  ...
  return (
    <section className="relative py-20 bg-gradient-to-br from-amber-50/70 via-rose-50/30 to-amber-50/40 overflow-hidden">
      {/* Elegant Background Elements */}
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-20 left-10 w-72 h-72 rounded-full bg-rose-200/20 blur-3xl animate-float"></div>
        <div className="absolute bottom-20 right-16 w-80 h-80 rounded-full bg-amber-200/20 blur-3xl animate-float-delay"></div>
        <div className="absolute inset-0 opacity-[0.02] bg-[url('data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iNjAiIGhlaWdodD0iNjAiIHZpZXdCb3g9IjAgMCA2MCA2MCIgeG1sbnM9Imh0dHA6Ly93d3cudzMub3JnLzIwMDAvc3ZnIj48ZyBmaWxsPSJub25lIiBzdHJva2U9IiNmZmYiIHN0cm9rZS13aWR0aD0iMSI+PGNpcmNsZSBjeD0iMzAiIGN5PSIzMCIgcj0iMjgiLz48L2c+PC9zdmc+')]"></div>
      </div>
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10 max-w-7xl">
        ...
        <div className={`text-center mb-16 transition-all duration-1000 ${isLoaded ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
          <div className="inline-flex items-center gap-2 mb-4 px-4 py-2 bg-white/80 backdrop-blur-sm rounded-full border border-rose-200 shadow-sm">
            <Heart className="w-4 h-4 text-rose-600" />
            <span className="text-sm font-semibold text-rose-700 tracking-wide uppercase">Featured Profiles</span>
            <Star className="w-4 h-4 text-rose-600" />
          </div>
          
          <h2 className="text-4xl md:text-5xl font-serif font-bold mb-4 text-slate-800 leading-tight">
            Meet Your Perfect
          ...
          <p className="text-lg text-slate-600 max-w-2xl mx-auto leading-relaxed">
            Discover verified profiles of amazing individuals ready for meaningful connections
          </p>
        </div>
        ...
        <div className="relative">
          ...
          <button 
            onClick={scrollLeft}
            disabled={currentStartIndex === 0}
            className="absolute left-0 top-1/2 -translate-y-1/2 z-30 bg-white/90 backdrop-blur-sm rounded-full p-3 shadow-lg hover:shadow-xl transition-all duration-300 md:-left-6 hover:scale-110 border border-rose-100 disabled:opacity-50"
          >
          ...
          <button 
            onClick={scrollRight}
            disabled={currentStartIndex >= maxStartIndex}
            className="absolute right-0 top-1/2 -translate-y-1/2 z-30 bg-white/90 backdrop-blur-sm rounded-full p-3 shadow-lg hover:shadow-xl transition-all duration-300 md:-right-6 hover:scale-110 border border-rose-100 disabled:opacity-50"
          >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {getVisibleProfiles().map((profile, index) => {
              ...
              return (
                ...
                >
                  <div className={`
                    relative bg-white rounded-2xl overflow-hidden shadow-md hover:shadow-xl 
                    transition-all duration-500 border border-rose-100/50
                    ${hoveredCard === actualIndex ? 'ring-2 ring-rose-300/50 shadow-rose-200/20' : ''}
                  `}>
                    
                    {/* Status Badges */}
                    <div className="absolute top-3 left-3 z-20 flex flex-col gap-2">
                      {profile.verified && (
                        <div className="flex items-center gap-1 px-2 py-1 bg-rose-600 text-white text-xs font-medium rounded-full shadow-sm">
                    ...
                    {/* Match Percentage */}
                    <div className="absolute top-3 right-3 z-20 px-2 py-1 bg-amber-500 text-white text-xs font-bold rounded-full shadow-sm">
                    ...
                    <div className="relative h-64 overflow-hidden">
                      ...
                      <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 flex gap-3 opacity-0 group-hover:opacity-100 transition-all duration-500 scale-90 group-hover:scale-100">
                        ...
                        >
                          <Heart size={20} fill={likedProfiles.has(actualIndex) ? 'currentColor' : 'none'} />
                        </button>
                        
                        <button 
                          onClick={() => handleViewProfile(actualIndex)}
                          className="p-3 bg-white/30 backdrop-blur-sm text-white rounded-full hover:bg-white/50 transition-all duration-300"
                        >
                          <Eye size={20} />
                        </button>
                      </div>
                    ...
                    {/* Profile Info */}
                    <div className="p-5">
                      <h3 className="text-xl font-serif font-semibold text-slate-800 mb-1">
                      ...
                      <button 
                        ...
                        className={`
                          ...
                          ${viewedProfiles.has(actualIndex) 
                            ...
                            : 'bg-gradient-to-r from-rose-500 to-amber-500 text-white hover:from-rose-600 hover:to-amber-600 hover:shadow-lg hover:-translate-y-0.5'
                          }
                        `}
                      >
                        {viewedProfiles.has(actualIndex) ? 'Profile Viewed' : 'View Profile'}
