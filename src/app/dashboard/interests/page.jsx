...
//sample
  ...
  const InterestCard = ({ person, type }) => {
    ...
    return (
      <motion.div 
        className="bg-white rounded-xl p-6 shadow-lg border border-orange-50 hover:shadow-xl transition-all duration-300 hover:border-orange-200"
        initial={{ opacity: 0, y: 10 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.3 }}
      >
        <div className="flex flex-col md:flex-row items-start space-y-4 md:space-y-0 md:space-x-6">
          ...
          <div className="relative flex-shrink-0 w-full md:w-auto">
            <motion.div 
              className={`w-32 h-32 mx-auto md:mx-0 bg-gradient-to-br from-orange-100 to-rose-100 rounded-full flex items-center justify-center overflow-hidden cursor-pointer hover:ring-2 hover:ring-rose-300 transition-all ${
                !hasSubscription && profileImage ? 'blur-sm' : ''
              }`}
              onClick={() => {
                if (!hasSubscription) {
            ...
            {profile?.isOnline && (
              <motion.div 
                className="absolute bottom-0 right-0 md:right-4 w-4 h-4 bg-green-500 rounded-full border-2 border-white"
                initial={{ scale: 0 }}
                animate={{ scale: 1 }}
                transition={{ type: "spring" }}
              />
            ...
            <div className="flex flex-col md:flex-row justify-between items-start mb-4 space-y-2 md:space-y-0">
              <div className="text-center md:text-left w-full">
                <div className="flex items-center justify-center md:justify-start space-x-2 mb-2">
                  ...
                  {profile?.badges?.includes('Verified') && <Shield className="w-5 h-5 text-green-500" />}
                </div>
                <div className="flex flex-wrap justify-center md:justify-start items-center space-x-4 text-sm text-slate-600 mb-2 gap-y-1">
                  <span className="flex items-center">
                ...
                <div className="flex flex-wrap justify-center md:justify-start items-center text-sm text-slate-600 gap-x-4 gap-y-1">
                  {profile?.currentCity && (
                    <span className="flex items-center">
              ...
              <div className="w-full md:w-auto mt-2 md:mt-0 self-center md:self-start">
                {getStatusBadge(person.status)}
              </div>
            </div>

            {/* Actions */}
            <div className="flex flex-col items-center md:items-start pt-4 border-t border-orange-100 space-y-2">
              ...
              <div className="flex flex-wrap justify-center md:justify-start gap-3">
                {type === 'received' && person.status === 'pending' && (
                  <>
                    ...
                    <motion.button 
                      onClick={() => handleInterestAction('accepted', person._id)}
                      className="flex items-center px-4 py-2 bg-green-50 text-green-600 rounded-lg text-sm font-medium hover:bg-green-100 transition-colors"
                      whileHover={{ scale: 1.03 }}
                      whileTap={{ scale: 0.97 }}
                    >
                ...
                {(type === 'sent' || type === 'received')  && (
                  <motion.button 
                    onClick={() => handleViewProfile(person, type)}
                    className="flex items-center px-4 py-2 bg-orange-50 text-orange-600 rounded-lg text-sm font-medium hover:bg-orange-100 transition-colors"
                    whileHover={{ scale: 1.05 }}
                    whileTap={{ scale: 0.95 }}
                    disabled={checkingSubscription}
                  >
  ...
  const ProfileDetailItem = ({ icon: Icon, label, value }) => (
    ...
    >
      ...
      <div>
        ...
        <div className="text-slate-800">{value || 'Not specified'}</div>
      </div>
    </motion.div>
  );
  ...
  if (isLoading && !isRefreshing) {
    return (
    <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-rose-50 via-white to-purple-50">
      <div className="text-center">
        {/* Simple Spinner */}
        <div className="w-12 h-12 border-4 border-orange-200 border-t-orange-600 rounded-full animate-spin mx-auto mb-4"></div>
        
        {/* Loading Text */}
        <p className="text-slate-600 text-lg">Loading your Interests</p>
      </div>
    </div>
    );
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-rose-50 via-white to-purple-50">
        <div className="text-center">
          ...
          <h3 className="text-lg font-medium text-slate-800 mb-2">Error loading interests</h3>
          <p className="text-slate-600 mb-4">{error}</p>
          <button
            onClick={handleRefresh}
            className="bg-gradient-to-r from-orange-500 to-rose-600 text-white px-6 py-2 rounded-lg font-medium hover:from-rose-500 hover:to-rose-600 transition-colors"
          >
            Try Again
          </button>
        </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-rose-50 via-white to-purple-50 p-6">
      ...
      <AnimatePresence>
        {expandedImage && (
          ...
          >
            ...
            >
              {/* Header */}
              <div className="bg-gradient-to-r from-orange-500 to-rose-600 p-2 flex justify-between items-center">
                <div className="flex items-center space-x-2">
                  <Heart className="w-5 h-5 text-white" />
                  <span className="text-white font-medium">Profile Picture</span>
                </div>
      ...
      <div className="max-w-5xl mx-auto space-y-6">
       <motion.div 
          className="bg-gradient-to-r from-orange-500 to-rose-600 rounded-2xl p-8 shadow-xl relative overflow-hidden text-white"
          initial={{ y: -20, opacity: 0 }}
          animate={{ y: 0, opacity: 1 }}
          transition={{ delay: 0.1 }}
        >
          ...
          <div className="relative z-10">
            <div className="flex flex-col md:flex-row items-center justify-between gap-4">
              <div>
                ...
                <p className="text-rose-100">Manage your sent and received interests</p>
              </div>
              
              <div className="flex space-x-6">
                <div className="text-center">
                  <div className="text-sm text-rose-100">Pending Received</div>
                  <div className="text-lg font-bold">{stats.pendingReceived || 0}</div>
                </div>
                <div className="text-center">
                  <div className="text-sm text-rose-100">Pending Sent</div>
        ...
        <motion.div 
          className="bg-white rounded-xl shadow-lg border border-purple-50"
          initial={{ y: 20, opacity: 0 }}
          animate={{ y: 0, opacity: 1 }}
          transition={{ delay: 0.2 }}
        >
  ...
  {showModal && selectedProfile && (
    ...
    >
      ...
      >
        {/* Modal Header */}
        <div className="sticky top-0 bg-gradient-to-r from-orange-500 to-rose-600 p-4 z-10 text-white">
          <div className="flex justify-between items-center">
        ...
        <div className="overflow-y-auto flex-1 p-6 space-y-6">
          ...
          <div className="flex items-center mb-6 space-x-6">
            <div className="relative">
              <div className="w-24 h-24 bg-gradient-to-br from-orange-100 to-rose-100 rounded-full flex items-center justify-center overflow-hidden">
                <img 
                  src={selectedProfile?.image || selectedProfile?.profilePhoto} 
                  alt={selectedProfile.name} 
                  className="w-full h-full object-cover"
            ...
            <div>
              ...
              <div className="flex items-center text-slate-600 mb-2 space-x-4">
                <span>{calculateAge(selectedProfile.dob)} years</span>
                <span>{selectedProfile.height}</span>
              </div>
            </div>
          </div>
          ...
          <ProfileSection title="Astrological Details" sectionKey="astro">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <ProfileDetailItem icon={Star} label="Rashi" value={selectedProfile.rashi} />
              <ProfileDetailItem icon={Star} label="Nakshatra" value={selectedProfile.nakshira} />
              <ProfileDetailItem icon={Star} label="Gothra" value={selectedProfile.gothra} />
