...
export default function MatchesPage() {
  ...
  const checkSubscription = async () => {
    try {
      ...
      const isActive = user?.subscription?.isActive || user?.user?.subscription?.isActive;
  ...
  const fetchUsers = async (pageNum = 1) => {
    try {
      ...
      const sentReceiverIds = await fetchSentInterests(currentUserData.id);
      ...
      // Quick Filters -> API Params
      if (quickFilters.verified) params.append('isVerified', 'true');
      if (quickFilters.withPhoto) params.append('hasPhoto', 'true');

      if (quickFilters.education) params.append('education', quickFilters.education);
      if (quickFilters.income) params.append('income', quickFilters.income);

      // Age Range
      if (quickFilters.ageRange[0] !== null) params.append('minAge', quickFilters.ageRange[0]);
      if (quickFilters.ageRange[1] !== null) params.append('maxAge', quickFilters.ageRange[1]);

      // Nearby Tab -> City Filter
      if (activeTab === 'nearby' && currentUserData?.currentCity) {
        params.append('city', currentUserData.currentCity);
      }

      const res = await fetch(`/api/users/fetchAllUsers?${params.toString()}`);
      const data = await res.json();
  ...
  const handleLoadMore = () => {
    if (page < totalPages) {
      ...
      setPage(nextPage);
      // Fetch users will use current filters state
      fetchUsers(nextPage);
    }
  };
  ...
  }, [searchQuery, quickFilters, activeTab]);

  const tabs = [
    { id: 'all', label: 'All Matches', count: matches.length, icon: Users },
    { id: 'preferred', label: 'Preferred', count: matches.filter(m => m.compatibility >= 70).length, icon: Star },
    // New and Nearby are now API driven or post-filtered? 
    // Nearby is API driven (city filter). 'New' is still mock.
    { id: 'new', label: 'New', count: matches.filter(m => m.isNew).length, icon: Sparkles },
    { id: 'nearby', label: 'Nearby', count: matches.length, icon: Navigation }
  ];

  // CLIENT-SIDE FILTERING (Only for things API can't do yet)
  const filteredMatches = matches.filter(match => {
    ...
    // Client-side 'activeRecently' mock check
            ...
            {match.mutualMatch ? (
              ...
              <button
                ...
                className={`flex-1 py-2 px-3 rounded-lg text-sm font-medium flex items-center justify-center transition-all shadow-sm group ${checkingSubscription
                  ...
                  : 'bg-gradient-to-r from-secondary to-primary text-gray-900 hover:shadow-md hover:scale-[1.02]'
                  }`}
              >
    ...
    <div className="min-h-screen bg-gradient-to-br from-rose-50/50 via-white to-amber-50/30">
      ...
      <div className="max-w-6xl mx-auto p-4 sm:p-6">
        ...
        <div className={`transform transition-all duration-1000 ${isLoaded ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`}>
          <div className="bg-gradient-to-r from-orange-500 via-rose-500 to-pink-600 rounded-xl p-4 sm:p-6 text-white shadow-xl relative overflow-hidden mb-4 sm:mb-6">
            ...
            <div className="relative z-10">
              ...
              <p className="text-white/90 text-sm sm:text-base">Discover profiles selected just for you</p>
            </div>
          </div>
        </div>
